...
class ResidualVectorQuantizer(torch.nn.Module):
    ...
    def initialize_codebooks(self, dataloader, encoder, device, n_batches=10):
        ...
        with torch.no_grad():
            for i, (_, _, x) in enumerate(dataloader):
                if i >= n_batches:
                    break
    ...
    def forward(self, x):
        ...
        for codebook in self.codebooks:
            ...
            rvq_loss += ((residual.detach() - closest_centroids)**2).sum(dim = -1).mean()
            rvq_loss += self.weight * ((residual - closest_centroids.detach())**2).sum(dim = -1).mean()
